...
export async function renderOverview() {
  ...
  document.getElementById('overview').innerHTML = `
    ...
    <div class="row">
      <button id="showCardsBtn">📋 Přehled</button>
      <button class="secondary" id="showStatsBtn">📊 Statistika</button>
    </div>
  ...
  document.getElementById('showCardsBtn').addEventListener('click', () => {
    renderCards(users, entries, map, dates, plan, exercises);
  });

  document.getElementById('showStatsBtn').addEventListener('click', () => {
    renderStatistics(users, map, dates, plan, exercises);
  });

  renderCards(users, entries, map, dates, plan, exercises);
}

function renderCards(users, entries, map, dates, plan, exercises) {
...
function renderStatistics(users, map, dates, plan, exercises) {
  let totalPercent = 0;

  let html = `
    <div class="card" style="margin-top:18px">
      <h3>📊 Statistika plnění</h3>
      <p class="note">Zelená = 70–100 %, oranžová = částečně, červená = bez zápisu.</p>
  ...
  users.forEach(user => {
    ...
    dates.forEach(date => {
      const value = completion(map[entryKey(user.id, date, plan.id)], exercises);
      sum += value;
      html += `<td class="${cellClass(value)}">${value ? Math.round(value * 100) : ''}</td>`;
    });

    const avg = Math.round(sum / (dates.length || 1) * 100);
    totalPercent += avg;

    html += `<td><b>${avg} %</b></td></tr>`;
  });

  const teamAverage = Math.round(totalPercent / (users.length || 1));

  html += `
        </table>
      </div>

      <p><b>Celkový průměr:</b> ${teamAverage} %</p>
    </div>
  `;
...
function renderEditForm(entry, exercises) {
  ...
  document.getElementById('editBox').innerHTML = `
    <div class="card" style="margin-top:18px;background:#f9fafb">
      ...
      <p class="note">Všechna cvičení se zadávají jako celé číslo. U Planku zapisujte počet sekund.</p>
